...
/*
This is a modified version of the hashtable implementation from my CUtils library.
! Don't try to use it as the original one, it's not the same. !
...
long int hm_get(hashtable *hm, char *key)
{
    int index = hm_hash(hm,key);
    item x = hm->items[index];
    if (x.data == NULL) return (long int) NULL;
...
hashtable* hm_init(struct pair* kvlist,int size) {
    ...
    for (int i = 0; i < size; i++) {
        hm_add(hm, kvlist[i].key, kvlist[i].value);
    }
    return hm;
}
